...
class MobileNetV2(nn.Module):
    def __init__(self, in_chans, downsample_factor=8, pretrained=True):
        super(MobileNetV2, self).__init__()

        model = mobilenetv2(pretrained=pretrained, in_chans=in_chans)
    ...
    def _nostride_dilate(m, dilate):
        classname = m.__class__.__name__      #用self.__class__将实例变量指向类，然后再去调用__name__类属性
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, in_chans, backbone='mobilenet', pretrained=True, downsample_factor=16, init_weights=True):
        super(DeepLab, self).__init__()
        if backbone == 'mobilenet':
            self.backbone = MobileNetV2(downsample_factor=downsample_factor, pretrained=pretrained, in_chans=in_chans)
            in_channels = 320
        ...
        self.lowlevel_conv = nn.Conv2d(48, num_classes, kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        low = self.lowlevel_conv(low_level_feature)
        low_feature = F.interpolate(low, size=(H, W), mode='bilinear', align_corners=True)
        ...
        outputs.append(x)
        # outputs.append(low_feature)
        return outputs


if __name__ == '__main__':

    model = DeepLab(num_classes=2,pretrained=False, in_chans=3)
