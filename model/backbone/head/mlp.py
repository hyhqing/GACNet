...
import torch
from model.backbone.head.base_decoder import BaseDecodeHead, resize
...
class MLPHead(BaseDecodeHead):
    ...
    def __init__(self, in_channels=[48, 64, 128, 256], channels=256, num_classes=2, in_index=[0, 1, 2, 3]):
...
if __name__ == '__main__':
    model = MLPHead()
    input_1 = torch.randn(size=(8, 256, 8, 8))
    input_2 = torch.randn(size=(8, 128, 16, 16))
    input_3 = torch.randn(size=(8, 64, 32, 32))
    input_4 = torch.randn(size=(8, 48, 64, 64))

    # output = model(input_1, input_2, input_3, input_4)

    from tool.flops_params_fps_count import flops_params_fps_1

    flops_params_fps_1(model, input_1_shape=(8, 256, 8, 8), input_2_shape=(8, 128, 16, 16), input_3_shape=(8, 64, 32, 32), input_4_shape=(8, 48, 64, 64))