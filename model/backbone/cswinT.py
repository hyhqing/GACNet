...
class CSWin(nn.Module):
    ...
    """
    def __init__(self, img_size=224, in_chans=None, patch_size=4, embed_dim = 64, depth=[1,2,21,1], split_size = [1, 2, 7, 7],
                 num_heads = [2, 4, 8, 16],mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], patches_resolution=256//4, mlp_ratio=mlp_ratio,
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], patches_resolution=256//8, mlp_ratio=mlp_ratio,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], patches_resolution=256//16, mlp_ratio=mlp_ratio,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], patches_resolution=256//32, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
...
class CSwin(nn.Module):
    ...
    def __init__(self, nclass, img_size, in_chans, embed_dim, depth, num_heads, split_size, drop_path_rate,
                 head_dim, pretrained_root=None, aux=False, head='mlphead'):
        ...
        if self.head_name == 'seghead':
            self.decode_head = seg.SegHead(in_channels=head_dim, num_classes=nclass, in_index=[0, 1, 2, 3])

        if self.head_name == 'mlphead':
            self.decode_head = mlp.MLPHead(in_channels=head_dim, num_classes=nclass, in_index=[0, 1, 2, 3], channels=256)

        if self.head_name == 'fcfpnhead':
            self.decode_head = fcfpn.FCFPNHead(in_channels=head_dim, num_classes=nclass, in_index=[0, 1, 2, 3], channels=256)

        # if self.aux:
        #     self.auxiliary_head = fcn.FCNHead(num_convs=1, in_channels=head_dim[2], num_classes=nclass, in_index=2, channels=256)

        if self.aux:
            self.auxiliary_head = mlp.MLPHead(in_channels=head_dim, num_classes=nclass, in_index=[0, 1, 2, 3], channels=256)
...
def cswin_tiny(nclass, img_size, in_chans, pretrained=False, aux=False, head='mlphead'):
    if pretrained:
        pretrained_root = './pretrained_weights/cswin_tiny_224.pth'
    else:
        pretrained_root = None
    model = CSwin(nclass=nclass, img_size=img_size, in_chans=in_chans, embed_dim=64, depth=[1, 2, 21, 1], split_size=[1, 2, 8, 8],
                  num_heads=[2, 4, 8, 16], head=head, pretrained_root=pretrained_root, aux=aux,
                  head_dim=[64, 128, 256, 512], drop_path_rate=0.3)
    return model


def cswin_small(nclass, img_size, in_chans, pretrained=False, aux=False, head='mlphead'):
    if pretrained:
        pretrained_root = './pretrained_weights/cswin_small_224.pth'
    else:
        pretrained_root = None
    model = CSwin(nclass=nclass, img_size=img_size, in_chans=in_chans, embed_dim=64, depth=[2, 4, 32, 2], split_size=[1, 2, 8, 8],
                  num_heads=[2, 4, 8, 16], head=head, pretrained_root=pretrained_root, aux=aux,
                  head_dim=[64, 128, 256, 512], drop_path_rate=0.4)
    return model


def cswin_base(nclass, img_size, in_chans, pretrained=False, aux=False, head='mlphead'):
    if pretrained:
        pretrained_root = './pretrained_weights/cswin_base_384.pth'
        # pretrained_root = './pretrained_weights/cswin_base_224.pth'
    else:
        pretrained_root = None
    model = CSwin(nclass=nclass, img_size=img_size, in_chans=in_chans, embed_dim=96, depth=[2, 4, 32, 2], split_size=[1, 2, 16, 16],
                  num_heads=[4, 8, 16, 32], head=head, pretrained_root=pretrained_root, aux=aux,
                  head_dim=[96, 192, 384, 768], drop_path_rate=0.6)
    return model


def cswin_large(nclass, img_size, in_chans, pretrained=False, aux=False, head='mlphead'):
    if pretrained:
        pretrained_root = './pretrained_weights/cswin_large_384.pth'
        # pretrained_root = './pretrained_weights/cswin_large_22k_224.pth'
    else:
        pretrained_root = None
    model = CSwin(nclass=nclass, img_size=img_size, in_chans=in_chans, embed_dim=144, depth=[2, 4, 32, 2], split_size=[1, 2, 16, 16],
                  num_heads=[6, 12, 24, 48], head=head, pretrained_root=pretrained_root, aux=aux,
                  head_dim=[144, 288, 576, 1152], drop_path_rate=0.6)
    return model


if __name__ == '__main__':
    model = cswin_tiny(nclass=2, in_chans=3, img_size=256, aux=True, head='fcfpnhead')
