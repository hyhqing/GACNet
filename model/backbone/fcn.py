...
def make_layers(cfg, in_chans, batch_norm=False):
    layers = []
...
def vgg16(in_chans, **kwargs):

    model = VGG(make_layers(cfg['D'], in_chans=in_chans), **kwargs)

    return model


class FCN16s(nn.Module):
    def __init__(self, nclass, in_chans, backbone='vgg16', aux=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(FCN16s, self).__init__()
        self.aux = aux
        if backbone == 'vgg16':
            self.pretrained = vgg16(in_chans=in_chans).features
        else:
...
if __name__ == '__main__':
    ...
    model = FCN16s(nclass=2, in_chans=4, aux=True)
    inputs = torch.randn(size=(8, 4, 256, 256))
    outputs = model(inputs)
    flops_params_fps(model, input_shape=(8, 4, 256, 256))
